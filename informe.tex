...
\textit{Dados una secuencia de \texttt{n} fuerzas de maestros agua $(x_i, x_2, \dots, x_n)$, y dos números \texttt{k} y \texttt{B}, ¿existe una partición en \texttt{k} subgrupos $(S_1, S_2, \dots, S_k)$ tal que:}

$$
\sum_{i=1}^{k} \left( \sum_{x_j \in S_i} x_j \right)^2 \leq B
$$

\textit{y cada elemento $x_i$ esté asignado a solamente un grupo?}
...
\inputminted[linenos, firstline=1, lastline=32]{python}{codigo/certificador_eficiente.py}
...
Enunciamos el problema de decisión asociado: \textit{dado un conjunto \( A =\{a_1, a_2, \\ \ldots, a_n\} \), ¿existen dos subconjuntos \( A_1 \) y \( A_2 \) tales que \( \sum_{a \in A_1} a = \sum_{a \in A_2} a \)?}
...
\textit{Dado un conjunto de números \( S = \{s_1, s_2, \ldots, s_n\} \) y un número \( V \), queremos saber si hay un subconjunto de \( S \) cuya suma sea igual a \( V \).}
...
\begin{itemize}
    ...
    \item $ O \cup T' = S \cup \{s-2 \cdot V\} = S'$
\end{itemize}
...
Luego, existe un subset sum de valor $V$ en $S$ si y solo si existe un 2-Partition en $S'$:
    ...
    $\rightarrow$ Si existe un conjunto de números en $S$ que suman $V$, entonces los restantes en $S$ suman \(s - V\). Por lo tanto, existe una partición de \texttt{S'} en dos tal que cada partición suma \(s - V\): \( O = S \setminus T \) y \( T' = T \cup \{s - 2 \cdot V\} \). 
...
Para esta reducción, decidimos utilizar el problema del Balanceo de Carga (que llamaremos B.C.) al problema de la tribu del agua (que llamaremos T.A.). Es decir, nuestra reducción polinomial va a ser de la forma:
$$
BC \leq_p TA
$$
...
La versión de decisión de este problema dice: \textit{Dado un número $m$ de mquinas, una serie $n$ de trabajos, y un número $C$, determinar si existe un asignación de trabajos en las $m$ máquinas, tal que el máximo $T_i$ sea menor o igual a $C$.}
...
Para demostrar que BC es un problema NP-Completo, vamos a usar el pro-\\blema 2P (el cual demostramos que es NP completo en \nameref{subsec:partition}). Es decir, vamos a realizar la reducción
$$
2P \leq_p BC
$$

Nosotros partimos de una serie de números $S = \{a_1, a_2, \ldots, a_n \}$ que es la entrada de 2-Partition. Por cada número $a_i$ definimos un trabajo con duración $a_i$.  
Luego, por cada uno de los dos subconjunto $S_i$, definimos una máquina $m_i$. 
...
Podemos afirmar entones, que existe un 2-Partition de $S$ si y solo si existe un balanceo de carga con $T_i$ a lo sumo $C$.
...
Con esto mostramos que el problema BC es un problema NP-Completo. En la \hyperref[sec:np-completo-bc-desarrollo]{próxima sección} usaremos este hecho para demostrar que TA también pertenece a los problemas NP-Completos.
...
Para reducir el tiempo de uso de una máquina, se le tiene que dar alguno de sus trabajos a otra máquina. Cuantos más trabajos se puedan pasar a otra máquina (sin que esta se convierta en la nueva máquina de mayor duración), más se va a lograr minimizar el valor máximo de las maquinas.
...
Por cada trabajo $t_j$, definimos un maestro agua $x_i$, donde la habilidad será el tiempo de ejecución del mismo. Por cada máquina $m_i$ definimos un subgrupo $S_i$ ($m$ sería nuestro $k$).
...
Ejemplo: $A = \{1, 2, 3\}$. Realizando la transformación, $k = 2$, $B = (\frac{6}{2})^2 + (\frac{6}{2})^2 = 18$ y $maestros = \{(M1, 1), (M2, 2), (M3, 3)\}$. Existen múltiples combinaciones de valores para los dos subconjuntos, pero nos quedaremos con la que minimice la suma de los cuadrados, respetando que debe ser a lo sumo $B$,
\begin{enumerate}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Asignaciones: cada maestro debe ser asignado a un solo un grupo. Luego, $\sum_{\texttt{j=1}}^{k} X_{i,j} = 1 \quad \forall i$.
...
La cátedra \footnote{(con la ayuda del maestro Pakku)} planteaba el siguiente algoritmo de aproximación para el pro-\\blema:
...
En la línea \texttt{15} creamos los k grupos vacíos. Luego, en la línea \texttt{16} ordenamos los maestros por su habilidad/destreza, utilizando \href{https://svn.python.org/projects/python/trunk/Objects/listsort.txt}{Timsort}.
...
Además, el ciclo \texttt{for} tiene  una complejidad temporal de $\mathcal{O}(n \cdot k)$, ya que por cada maestro que tiene que ser añadido a un grupo, tiene que buscar el grupo con menor habilidad acumulada hasta ahora, utilizando la función \texttt{min}, lo cual es una operación $\mathcal{O}(k)$ adicional. 
...
Para corroborar la cota encontrada, $r(A) = 1.02$, también realizamos mediciones con mayores volúmenes de datos, con $n \in \{10, 100, 1000, 10000\}$. Obtuvimos $r(A) = 1.0256009216095232 \approx 1.0256$. Los ejemplos se pueden encontrar en \texttt{codigo/cota\_aprox\_catedra.py}
...
\inputminted[linenos, firstline=3, lastline=34]{python}{codigo/aproximacion_adicional.py}
...
El análisis de la cota es presentado en \nameref{sec:cota}.
...
\begin{itemize}
    \item \texttt{python3\ codigo/main.py}: ejecutará por defecto el algoritmo de backtracking y mostrará los grupos formados, el coeficiente resultante y el tiempo de ejecución en cada caso. Se ejecutan los ejemplos adicionales, los de las mediciones y los provistos por la cátedra. 
...
\begin{itemize}
    \item Programación lineal $\rightarrow$ $r(A) = 1.0141856654211776 \approx 1.01419$. En gene-\\ral, los resultados coincidieron con el óptimo. En otros casos, la diferencia entre las soluciones resultó ser muy pequeña.
    \item Aproximación de la cátedra $\rightarrow$ Con el análisis presentado en \nameref{sec:cota-catedra}, $r(A) = 1.0256009216095232 \approx 1.03$.
...
\includegraphics[scale=0.60]{images/graficoAproxCatedraMediciones.png}
...
Hemos demostrado que el Problema de la Tribu del Agua es NP-completo, lo que implica que no se conoce \footnote{(todavía)} un algoritmo para resolverlo de manera exacta en tiempo polinomial. Los algoritmos exactos, como el presentado para backtracking, conllevan tiempo exponencial, es decir, el tiempo necesario para obtener la solución aumenta rápidamente a medida que crece el tamaño del problema, con $n$ y $k$. Esto subraya la importancia de buscar soluciones aproximadas eficientes con un menor costo computacional.
